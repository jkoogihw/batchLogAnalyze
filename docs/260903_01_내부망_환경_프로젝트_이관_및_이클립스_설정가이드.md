...
### [Step 1] 외부망: GitHub 저장소 패키징 및 소스 다운로드

> **안내:** 프로젝트 저장소(`main` 브랜치)에 **61개 의존성 JAR(`libs/`)**, **Eclipse 메타데이터(`.project`, `.classpath`)**, **원클릭 실행 파일(`.launch`)**이 **모두 커밋되어 포함**되어 있습니다.  
> 따라서 외부망에서 GitHub 소스코드를 Clone하거나 ZIP으로 다운로드받는 것만으로 모든 필수 라이브러리와 설정이 100% 함께 다운로드됩니다.

```powershell
# 1. GitHub에서 전체 프로젝트 소스코드 Clone (libs 61개 JAR 및 Eclipse 설정 자동 포함)
cd D:\dev\workspace
git clone https://github.com/jkoogihw/batchLogAnalyze.git

# (선택 사항) 웹 브라우저에서 'Download ZIP'으로 다운로드해도 동일하게 모든 JAR가 포함됨
```

- **저장소에 커밋되어 포함된 핵심 자산:**
  - `libs/*.jar`: Spring Boot 2.3.12, JUnit 5 등 필수 라이브러리 61개
  - `.project`, `.classpath`: `libs/` 상대 경로가 매핑된 Eclipse 전용 프로젝트 파일
  - `CheckLog.launch`, `CheckLogTest.launch`: Eclipse 원클릭 GUI 실행 구성 파일
  - `gradle/wrapper/gradle-wrapper.jar`: Gradle 래퍼 바이너리

---

### [Step 2] 망연계: 내부망으로 소스코드 이관 및 압축 해제

1. GitHub에서 다운로드한 `batchLogAnalyze` 프로젝트 폴더를 사내 망연계 솔루션(또는 승인된 반입 매체)을 통해 내부망 개발 PC로 반입합니다.
2. 내부망 PC의 원하는 작업 디렉토리에 배치합니다.  
   *(예: `C:\workspace\batchLogAnalyze` 또는 `D:\dev\workspace\batchLogAnalyze` 등)*
...
| :--- | :--- | :--- |
| 2026-09-03 10:00:00 | 내부망 오프라인 환경 이관 및 Eclipse 설정, 61개 JAR 상대경로 자립화 가이드 작성 | [`89bf863`](https://github.com/jkoogihw/batchLogAnalyze/commit/89bf863) |
